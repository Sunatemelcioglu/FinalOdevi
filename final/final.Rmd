...
```
## Özet İstatistikler
Özet istatistikler tabloda (Tablo 1) gösterilmiştir.
```{r}
sayisal_sutunlar <- superbowl_data[, sapply(superbowl_data, is.numeric)]
ozet_istatistikler <- apply(sayisal_sutunlar, 2, function(x) {
  c(Minimum = min(x), Quartile1 = quantile(x, 0.25), Medyan = median(x), Quartile3 = quantile(x, 0.75),
    Ortalama = mean(x), Maksimum = max(x))
})
tablo <- data.frame(
  Değişkenler = names(sayisal_sutunlar),
  Minimum = ozet_istatistikler[1, ],
  Quartile1 = ozet_istatistikler[2, ],
  Medyan = ozet_istatistikler[3, ],
  Quartile3 = ozet_istatistikler[4, ],
  Ortalama = ozet_istatistikler[5, ],
  Maksimum = ozet_istatistikler[6, ]
)
tablo
...
Şekil 1: Basit Doğrusal Regresyon
  
Bu araştırma sırasında wins ve points for değişkeni arasında basit doğrusal regresyon analizi yapılmıştır. Grafik şekilde gösterilmiştir. (Şekil 1) Yapılan regresyon analizi sonucunda wins değişkeninin points for değişkeni üzerinde anlamlı bir etkiye sahip olduğu gözlemlenmiştir. Katsayılar bölümünde wins değişkeninin eğim (slope) katsayısı 16.9037 olarak hesaplanmıştır bu katsayı her bir ekstra kazanılan maç için ortalama olarak puanların 16.9037 artacağını ifade etmektedir. Ayrıca, kesme noktası (intercept) katsayısı olan Intercept için tahmin edilen değer 215.3192'dir ki bu katsayı da kazanılan maç sayısı sıfır olduğunda beklenen ortalama puanın 215.3192 olduğunu ifade etmektedir. Analize ayrıca R-kare (R-squared) değeri 0.533 olarak hesaplanmıştır, yani modelin açıkladığı varyansın %53.3'ünü wins değişkeni açıklamaktadır ki bu modelin veri setindeki değişikliğin büyük bir kısmını açıkladığını göstermektedir. Son olarak, analiz neticesinde F istatistiği 726 olarak hesaplanmış ve p-değeri < 2.2e-16 (çok düşük) olarak bulunmuştur. Bu da, wins değişkeninin puanları açıklamada istatistiksel olarak anlamlı olduğunu göstermektedir. Bu sonuçlar, wins ve points for değişkeni arasında pozitif bir ilişki olduğunu ve kazanılan her maçın takımın puanlarını artırdığını göstermektedir. 
...
Şekil 2: Scatter Plot

Bu doğrusal regresyonda kazanılan maçlar (superbowl_data$wins) ve attığı basket sayısı (points_for) değişkeni üzerinde anlamlı bir etkiye sahiptir.Bu sonuçlar, kazanılan maçlar ve attığı puanlar arasında pozitif bir ilişki olduğunu ve kazanılan her maçın takımın puanlarını artırdığını göstermektedir. Şekil 2'de grafik görülmektedir.
...
Şekil 3: Playoff'a Kalan Takımların Toplam Galibiyetleri Sütun Grafiği
...
y ekseninde frekans, x ekseninde strength of schedule değişkeni yer almaktadır. Grafik şekilde görülmektedir. (Şekil 4)
